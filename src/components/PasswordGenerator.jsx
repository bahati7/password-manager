...
function PasswordGenerator() {
  ...
  const passwordRef = useRef(null); // Create a ref to store the password element
  ...
  const generatePassword = () => {
    ...
    let allChars = lowercaseLetters + uppercaseLetters + numbers + symbols;
    let newPassword = "";
    ...
    for (let i = newPassword.length; i < 15; i++) {
  ...
  const copyPassword = () => {
    const text = passwordRef.current.innerText; // Get the text content of the password element
    navigator.clipboard.writeText(text) // Use clipboard API to copy the text
      .then(() => {
        if(!text){
      ...
      .catch((err) => {
        console.error("Failed to copy password:", err); // Optional error handling
      });
  };
  ...
  return (
    <div className="password-generator">
      ...
      <kbd className="px-2 py-1.5 text-xs font-semibold text-gray-800 bg-gray-100 border border-gray-200 rounded-lg dark:bg-gray-600 dark:text-gray-100 dark:border-gray-500">Click bellow to copy the password</kbd>
      <label htmlFor="password"> </label>
      <li ref={passwordRef} onClick={copyPassword}>
          <a
            href="#"
      ...
      <button className="w-full text-white bg-green-400 hover:bg-yellow-600 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800" onClick={generatePassword}>Generate Password</button>
    </div>
  );
}
